...
# Create a dedicated database service class
...
class TankMonitor:
    ...
    def __init__(self, tank_id, app, thread_class, db_service=None):
        ...
        # Initialize measurement processor with tank config
        ...
        # Initialize flow rate calculator with parameters suitable for both refill and consumption
        self.flow_calculator = FlowRateCalculator(
            max_history_points=5,
            min_significant_change=5.0,        # 5 liters minimum change for normal operations
            smoothing_factor=0.5,              # 50% smoothing factor
            expected_max_inflow_rate=600.0,    # Maximum refill rate (positive): 600 L/min
            expected_max_outflow_rate=100.0,   # Maximum consumption rate (negative): 100 L/min
            refill_detection_threshold=100.0   # Threshold to detect refill operations
        )
        
        # Initialize pressure readings for smoothing
    ...
    def load_config(self):
        ...
        with self.app.app_context():
            tank = Tank.query.get(self.tank_id)
            if not tank:
            ...
            # Store alarm thresholds for later use
            self.alarm_thresholds = {
                'low_level_threshold': tank.low_level_threshold,
                'critical_level_threshold': tank.critical_level_threshold,
                'high_level_threshold': tank.high_level_threshold
    ...
    def connect(self):
        ...
        if result:
            # Update tank connection status in database
    ...
    def read_measurement(self):
        ...
        try:
            # Read all sensor data in one method
            sensor_data = self._read_sensor_data()
            if not sensor_data:
                return None
            
            # Apply smoothing to pressure before processing
            smoothed_pressure = self.smooth_pressure(sensor_data['pressure'])
    ...
    def _read_sensor_data(self):
        ...
        try:
            ...
            temperature = None
            try:
                time.sleep(1)  # Add delay between readings
    ...
    def start_monitoring(self):
        ...
        with self.lock:
            ...
            # Use the Thread class that was injected
    ...
    def _monitor_thread(self):
        ...
        # Get the update interval from config
        update_interval = self.app.config.get('UPDATE_INTERVAL', 60)  # Default to 60 seconds
    ...
    def _handle_successful_measurement(self, measurement):
        ...
        # Additional processing for successful measurements could go here
    ...
    def _handle_failed_measurement(self, consecutive_failures, max_consecutive_failures):
        ...
        if consecutive_failures >= max_consecutive_failures:
            ...
            if self.connect():
                return 0  # Reset counter on successful reconnection
            else:
    ...
    def smooth_pressure(self, pressure):
        ...
        if not hasattr(self, 'pressure_readings'):
            self.pressure_readings = []
            self.max_pressure_readings = 5  # Increase from 5 to 10 for more stability
        ...
        if len(self.pressure_readings) >= 3:
            ...
            max_deviation = 0.002  # Reduce from 0.005 to 0.001 (0.1% deviation)
    ...
    def _calculate_flow_and_statistics(self, measurement_data):
        ...
        Returns:
            Updated measurement data with flow rate
        """
        try:
            # Get the latest measurement for flow rate calculation
            latest = self._get_latest_measurement()
        
            # Calculate flow rate using the flow calculator
            flow_rate = 0.0
            if 'volume' in measurement_data:
                # Check if volume is stable compared to previous measurement
                volume_stable = False
                if latest and 'volume' in latest:
                    prev_volume = latest['volume']
                    curr_volume = measurement_data['volume']
                
                    # If volume hasn't changed significantly (less than 0.5L), consider it stable
                    if abs(curr_volume - prev_volume) < 0.5:
                        # If the flow calculator already has a flow rate, decay it
                        if self.flow_calculator.current_flow_rate != 0:
                            # Apply 70% decay (retain 30% of previous value)
                            decayed_rate = self.flow_calculator.current_flow_rate * 0.3
                            # If decayed rate is very small, set to zero
                        ...
                        # Volume changed, calculate actual flow rate
                        flow_rate = self.flow_calculator.add_measurement(
                            measurement_data['volume'], 
                            measurement_data['timestamp']
                        )
                else:
                    # No previous measurement, calculate flow rate normally
                    flow_rate = self.flow_calculator.add_measurement(
                        measurement_data['volume'], 
                        measurement_data['timestamp']
                    )
        
            # Add flow rate to measurement data
            ...
            # If there's an error, still return the original data but with zero flow rate
    ...
    def _get_latest_measurement(self):
        ...
        Returns:
            Latest measurement as a dictionary or None if no measurements exist
        """
        try:
            with self.app.app_context():
                tank = Tank.query.get(self.tank_id)
                ...
                # If the tank doesn't have a get_latest_measurement method, query directly
    ...
    def _save_measurement_to_db(self, measurement_data):
        """Save measurement to database and return as dictionary."""
        # Create measurement
        measurement = Measurement(
        ...
        # Update last_connection time in database
        tank = Tank.query.get(self.tank_id)
        if tank:
            tank.last_connection = datetime.now()
            tank.connection_status = 'connected'
        ...
        # Get a copy of the measurement data before closing the session
        return measurement.to_dict() if hasattr(measurement, 'to_dict') else dict(
    ...
    def calibrate(self, known_volume):
        ...
        try:
            ...
            with session_scope(self.app):
                ...
                # Try to read temperature (optional)
                temperature = None
                try:
                    temp_result = self.connection.read_channel_float(self.config.temp_channel)
                    if temp_result and not math.isnan(temp_result['value']):
                ...
                # Calculate uncalibrated level (with calibration factor = 1)
    ...
    def _get_level_hysteresis(self, tank_config):
        ...
        # Check if custom hysteresis is defined in database
        with self.app.app_context():
            tank = Tank.query.get(self.tank_id)
            if tank and hasattr(tank, 'level_hysteresis') and tank.level_hysteresis is not None:
                return tank.level_hysteresis
        
        # Default based on tank size
        # tank_volume is in cubic meters, so multiply by 1000 to get liters
