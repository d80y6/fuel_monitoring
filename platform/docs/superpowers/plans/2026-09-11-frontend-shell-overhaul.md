...
**Step 1.1 — Test (red).** Create `web/src/tests/tokenContract.test.ts` (short keys map to vars; same assertion style as config):
...
> Note: this repo's tsconfig restricts `types` and has no `@types/node`. If tsc fails on `node:fs`/`__dirname`, add `// @ts-expect-error` on the two node imports and the tsconfig `types` entry stays untouched — runtime resolution under vitest still works (verified during execution).
...
> Only short token keys go in `tailwind.config.js`. Do **not** add `--color-*`-keyed duplicates — the contract test matches short keys to `var(--color-*)` references. Commit message: `feat(web): design-token theme variables + tailwind mapping`.

(Read `tailwind.config.js` first; preserve the rest of the config verbatim.)
