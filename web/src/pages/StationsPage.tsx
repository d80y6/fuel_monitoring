...
export default function StationsPage() {
  ...
  const deleteMut = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['stations', siteId] }); setDeleting(null); },
  });

  function resetForm() {
    setShowForm(false); setEditing(null); setName(''); setSerialNumber(''); setRaspberryPiId(''); setFirmwareVersion('');
  }

  function openCreate() {
    setEditing(null); setName(''); setSerialNumber(''); setRaspberryPiId(''); setFirmwareVersion(''); setShowForm(true);
  }

  function openEdit(s: Station) {
    setEditing(s); setName(s.name); setSerialNumber(s.serial_number); setRaspberryPiId(s.raspberry_pi_id ?? ''); setFirmwareVersion(s.firmware_version ?? ''); setShowForm(true);
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <button type="button" onClick={openCreate} className="bg-indigo-600 text-white rounded px-3 py-2 text-sm font-medium">New station</button>
      </div>
          ...
          <tbody>
            {stations.map((s) => (
              <tr key={s.id} className="border-b last:border-0">
                <td className="py-2 pr-4"><Link to={`/dispensers/${s.id}`} className="text-indigo-600 hover:underline">{s.name}</Link></td>
                ...
                <td className="py-2 space-x-2">
                  <button type="button" onClick={() => openEdit(s)} className="text-indigo-600 hover:underline text-sm">Edit</button>
      ...
      {showForm && (
        <Modal title={editing ? 'Edit station' : 'New station'} onClose={resetForm}>
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            <Field label="Serial number" htmlFor="station-serial"><Input id="station-serial" value={serialNumber} onChange={(e) => setSerialNumber(e.target.value)} required /></Field>
            ...
            <div className="flex justify-end gap-2 pt-2">
              ...
              <button type="submit" disabled={createMut.isPending || updateMut.isPending} className="bg-indigo-600 text-white rounded px-3 py-2 text-sm font-medium">
                {editing ? 'Save' : 'Create'}
              </button>
            </div>
          </form>
